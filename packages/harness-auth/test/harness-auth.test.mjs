...
test("claude auth status: JSON, prose, and the two disagreeing", () => {
  ...
  assert.equal(expiredText.email, "user@example.com");
...
test("an auth failure is spotted and rewritten into one actionable line", () => {
...
test("rewriteHarnessOutput only rewrites what looksLikeAuthFailure caught", () => {
  const raw = "Failed to authenticate: OAuth session expired and could not be refreshed";
  assert.equal(rewriteHarnessOutput("claude", raw), friendlyHarnessFailure("claude", raw));
  assert.equal(rewriteHarnessOutput("claude", "Frontier $240 1-stop"), "Frontier $240 1-stop");
});

test("applyAuthAlert expires a remembered failure but never a live sign-in", () => {
