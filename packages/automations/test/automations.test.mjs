...
import { overlappingRoutine, upsertRoutine } from "../dist/index.js";

const NEW_YORK = "America/New_York";
// server/context.mjs resolveZone() hands the caller a validated IANA zone; this
// is the value it returns for { userTimeZoneOverride: "Europe/London" }.
...
test("automation json helper matches orchestration AutomationJson", () => {
  ...
  assert.equal(
    routines.automationPath("bot-1", routine),
    "/config/agent-data/agents/bot-1/automations/inbox/automation.json",
  );
  assert.equal(routines.automationSlug("X inbox"), "x-inbox");
});

test("calendar and cron automations serialize schedule as a string", () => {
  const morningJson = routines.automationJson({
    name: "Morning",
    instruction: "watch the inbox every morning",
    schedule: morning,
    enabled: true,
  ...
  assert.equal(morningJson.schedule, "0 9 * * *");
  assert.equal(morningJson.lastRunAt, null);
  const cronJson = routines.automationJson({
    name: "Weekday digest",
    instruction: "Send the digest.",
    triggers: [{ kind: "cron", cron: "0 8 * * 1-5" }],
    enabled: false,
    createdAt: 42,
    lastRunAt: 99,
  });
  assert.equal(cronJson.schedule, "0 8 * * 1-5");
  assert.equal(cronJson.enabled, false);
  assert.equal(cronJson.lastRunAt, 99);
  assert.equal(cronJson.createdAt, 42);
});

test("unnormalized schedule objects stay objects on disk", () => {
  const json = routines.automationJson({
    name: "Odd",
    instruction: "custom cadence",
    schedule: { type: "weekly", weekday: 1 },
    enabled: true,
    createdAt: 1,
    lastRunAt: null,
  });
  assert.deepEqual(json.schedule, { type: "weekly", weekday: 1 });
});
