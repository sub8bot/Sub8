...
export async function teamDeskPrompt(bot: ContextBot | null | undefined): Promise<string> {
  ...
  const role = lead
    ? [
        ...
        "Your final message is your reply to the user. On a turn where you hand work off, the delegation line is your reply and your closing text is not shown — put anything else the user should see in send_message. When a teammate reports back (“Nova replies: …”), the user already sees it in the channel: speak only if you add something — a combined answer, a next step, an unblock — and otherwise call nothing_to_add, which ends your turn quietly. Do not acknowledge or restate these instructions.",
