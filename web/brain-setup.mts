...
/** One harness as the server reports it. */
...
export const HARNESS_INSTALL: Record<string, HarnessInstall> = {
  "grok-build": {
    url: "https://x.ai/cli",
...
export const API_PRESETS: [ApiPreset, ...ApiPreset[]] = [
  {
    id: "spacexai",
    label: "xAI / Grok",
...
export function apiPreset(id: string | undefined): ApiPreset {
...
export function needsBrainSetup(
  settings: { harness?: HarnessSettings } | null | undefined,
  status: HarnessStatus | null | undefined,
): boolean {
  const h: HarnessSettings = settings?.harness || {};
...
export function harnessAction(info: HarnessInfo | null | undefined): { kind: string; label: string } {
...
export interface BrainApiDraft {
  preset?: string;
  baseUrl?: string;
  apiKey?: string;
  model?: string;
...
export interface BrainSetupView {
  tab?: string;
  harnesses?: Record<string, HarnessInfo>;
...
export function brainSetupHtml({
  ...
  const rows = (catalog.length ? catalog : HARNESS_SETUP_IDS.map((id) => ({ id, label: id })))
    ...
    .map((item) => {
      const info: HarnessInfo = harnesses[item.id] || { id: item.id, label: item.label, installed: false, ready: false };
      ...
      const install: Partial<HarnessInstall> = HARNESS_INSTALL[item.id] || {};
...
export function harnessSetupBannerHtml(info: Partial<HarnessInfo> | null | undefined): string {
  const id = info?.id || "";
...
export function applySimulate<T extends HarnessStatus | null | undefined>(status: T, simulate: string | null | undefined): T {
