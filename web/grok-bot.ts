...
function taperedTube(
  ...
  for (let i = 0; i <= tubular; i++) {
    ...
    // computeFrenetFrames(tubular) fills exactly tubular + 1 frames and this
    // loop runs i <= tubular, so both reads are in range.
  ...
  // Same frame count: index `tubular` is the last one, never past the end.
...
/** One eye's per-axis scale: x and y ride the expression, z is the flatten. */
interface EyeScale {
  x: number;
  y: number;
  z: number;
...
/** A tentacle curve: three control points, nine numbers, in x/y/z order. */
type Curve9 = [number, number, number, number, number, number, number, number, number];

/** Where an arm meets the body: [sideways spread, y, forward spread]. */
type Attach3 = [number, number, number];

/** Which of a body spec's three curves an arm follows. */
type ArmKind = "back" | "flare" | "hang";

/** One built tentacle and the angle it was grown at. */
interface Arm {
  root: THREE.Group;
  index: number;
  angle: number;
...
/** A face: everything face() fills in, plus the per-eye overrides some use. */
export interface FaceSpec {
  ...
  blink: boolean;
  blush: boolean;
  /** false for none, or a count: 1 tear on the left, 2 for both. */
  tears: boolean | number;
  feel: string;
  label: string;
  lEyeX?: number;
  lEyeY?: number;
...
/** The face catalog. Unknown names miss; the two fallbacks are always there. */
export interface FaceCatalog {
  [name: string]: FaceSpec;
  /** setExpression() falls back to this one. */
  neutral: FaceSpec;
  /** update()'s love beat falls back to this one. */
  love: FaceSpec;
}

/** A motion. The catalog is the list; the movement itself lives in _updateAnim. */
...
/** A silhouette: the head form plus the arms grown off it. */
export interface BodySpec {
  ...
  flare: Curve9;
  hang: Curve9;
  /** The curve the three above were seeded from, kept for the next override. */
  curve?: Curve9;
  tall?: number;
  belly?: number;
  crown?: number;
...
/** The body catalog. `rounder` is the fallback, so it is always present. */
export interface BodyCatalog {
  [name: string]: BodySpec;
  rounder: BodySpec;
}

/** What a caller may hand the constructor; DEFAULTS fills in the rest. */
...
/** Options after DEFAULTS: every measurement is known, `body` still optional. */
type ResolvedOptions = typeof DEFAULTS & Pick<GrokBotOptions, "body">;

/** The eye meshes, keyed by style. blush and tear are added by _buildCheeks. */
interface EyeGeos {
  stadium: THREE.BufferGeometry;
  round: THREE.BufferGeometry;
  crescent: THREE.BufferGeometry;
  line: THREE.BufferGeometry;
  heart: THREE.BufferGeometry;
  star: THREE.BufferGeometry;
  x: THREE.BufferGeometry;
...
/** The mouth geometries, all built together in _buildMouths. */
interface MouthGeos {
  dash: THREE.BufferGeometry;
  smile: THREE.BufferGeometry;
  frown: THREE.BufferGeometry;
  smirk: THREE.BufferGeometry;
  oh: THREE.BufferGeometry;
  grin: THREE.BufferGeometry;
  grimace: THREE.BufferGeometry;
  tongue: THREE.BufferGeometry;
  kiss: THREE.BufferGeometry;
  openFill: THREE.BufferGeometry;
  small: THREE.BufferGeometry;
  teeth: THREE.BufferGeometry;
  wavy: THREE.BufferGeometry;
}

/** placeEyes' knobs, each defaulted at the destructure. */
interface PlaceEyesOptions {
  tilt?: number;
  spacing?: number;
  elevation?: number;
  lTilt?: number;
...
function face(label: string, extra: Partial<FaceSpec> = {}): FaceSpec {
  return {
    tilt: 0,
    spacing: 0.4,
    elevation: 0.1,
...
export const ANIMATIONS: AnimationCatalog = {
  none: { label: 'Still' },
  idle: { label: 'Idle' },
  bounce: { label: 'Bounce' },
  hop: { label: 'Hop' },
...
function octoShape(label: string, extra: Partial<BodySpec> = {}): BodySpec {
  const curve: Curve9 = extra.curve || [0.2, -0.26, 0.18, 0.42, -0.5, 0.32, 0.4, -0.22, 0.42];
  return {
    label,
    kind: 'octopus',
...
function mantle(label: string, extra: Partial<BodySpec> = {}): BodySpec {
...
export function isOctopusBody(): boolean {
  return true;
}
...
export class GrokBot extends THREE.Group {
  // Every field is `declare`d: the constructor and the _build* helpers assign
  // them at runtime exactly as the JavaScript did, and a real class field would
  // define the property (as undefined) before that ever ran.
  declare radius: number;
  declare autoIdle: boolean;
  declare autoMouth: boolean;
  declare autoBlink: boolean;
  declare expression: string;
  declare animation: string;
  declare bodyType: string;
  declare bodyScale: THREE.Vector3;

  declare bodyMaterial: THREE.MeshPhysicalMaterial;
  declare eyeMaterial: THREE.MeshBasicMaterial;
  declare whiteMaterial: THREE.MeshBasicMaterial;
  declare pupilMaterial: THREE.MeshBasicMaterial;
  declare accentMaterial: THREE.MeshBasicMaterial;
  declare starMaterial: THREE.MeshBasicMaterial;
  declare tearMaterial: THREE.MeshBasicMaterial;
  declare blushMaterial: THREE.MeshBasicMaterial;
  declare fillMaterial: THREE.MeshBasicMaterial;
  // Never assigned anywhere; dispose() optional-chains past them. Kept so the
  // two `?.dispose()` calls it still makes keep meaning what they meant.
  declare bellyMaterial?: THREE.Material;
  declare suckerMaterial?: THREE.Material;

  declare body: THREE.Mesh;
  declare eyes: THREE.Group;
  declare eyeL: THREE.Group;
  declare eyeR: THREE.Group;
  declare blush: THREE.Group;
  declare blushL: THREE.Mesh;
  declare blushR: THREE.Mesh;
  declare tears: THREE.Group;
  declare tearL: THREE.Mesh;
  declare tearR: THREE.Mesh;
  declare mouths: THREE.Group;
  declare mouth: THREE.Mesh;
  declare mouthDash: THREE.Mesh;
  declare mouthSmile: THREE.Mesh;
  declare mouthFrown: THREE.Mesh;
  declare mouthSmirk: THREE.Mesh;
  declare mouthOh: THREE.Mesh;
  declare mouthGrin: THREE.Mesh;
  declare mouthGrimace: THREE.Mesh;
  declare mouthTongue: THREE.Mesh;
  declare mouthKiss: THREE.Mesh;
  declare mouthOpenFill: THREE.Mesh;
  declare mouthSmall: THREE.Mesh;
  declare mouthWavy: THREE.Mesh;
  declare mouthTeeth: THREE.Mesh;
  declare tentacles: THREE.Group | null;

  declare _feel: string;
  declare _bodySpec: BodySpec | null;
  declare _bodyGeo?: THREE.BufferGeometry;
  declare _eyeGeometry: THREE.CapsuleGeometry;
  declare _eyeGeos: EyeGeos;
  declare _geos: MouthGeos;
  declare _blink: number;
  declare _blinkTo: number;
  declare _blinkTimer: number;
  declare _doubleBlink: boolean;
  declare _mouth: number;
  declare _mouthTo: number;
  declare _mouthHold: boolean;
  declare _mouthTimer: number;
  declare _mouthShape: string;
  declare _elapsed: number;
  declare _baseBodyColor: THREE.Color;
  declare _flashColor: THREE.Color;
  declare _flatten: number;
  declare _winkL: boolean;
  declare _winkR: boolean;
  declare _eyeBase: { l: EyeScale; r: EyeScale };
  declare _talk: number;
  declare _arms: Arm[];
  declare _tentacleProfile: string | null;
  declare _tentacleGeos?: THREE.BufferGeometry[];
  declare _uniBase: null;

  // Set by update()/_updateAnim for the frame, read back later in the same
  // frame. Undefined until the matching feel or animation has run once.
  declare _snore?: number;
  declare _confusedGlance?: number;
  declare _scaredGulp?: number;
  declare _sadSigh?: number;
  declare _angryHuff?: number;
  declare _wowGasp?: number;
  declare _happyGiggle?: number;
  declare _winkLid?: number;
  declare _smirkTwitch?: number;
  declare _deadpanHold?: number;
  declare _deadpanLook?: number;
  declare _sleepyDroop?: number;
  ...
  _buildEyes(opts: ResolvedOptions): void {
    const r = opts.radius;
  ...
  _makeEye(side: string): THREE.Group {
  ...
  _setEyeStyle(group: THREE.Group, style: string): void {
    const name = group.userData.styles[style] ? style : 'stadium';
    // _makeEye is the only writer of userData.styles, and it stores the
    // Record<string, Object3D> built there.
    for (const [key, mesh] of Object.entries<THREE.Object3D>(group.userData.styles)) {
      mesh.visible = key === name;
    }
  }

  _buildCheeks(opts: ResolvedOptions): void {
    const r = opts.radius;
  ...
  _placeCheeks(): void {
    const r = this.radius;
    const put = (mesh: THREE.Mesh, x: number, y: number) => {
  ...
  _buildMouths(opts: ResolvedOptions): void {
  ...
  _pts(r: number, sx: number, cz: number, t: Curve9): [THREE.Vector3, THREE.Vector3, THREE.Vector3] {
    return [
  ...
  _shapeVertex(v: THREE.Vector3, spec: BodySpec | null): THREE.Vector3 {
    const r = this.radius;
    ...
    if (form === 'mantle') {
      // `form` came from spec?.form, so a null spec would have landed on
      // 'round' above and never reached here.
  ...
  _buildTentacles(profileName: string): void {
    ...
    for (let i = 0; i < list.length; i++) {
      // i < list.length, so the row is there.
  ...
  _evenArms(spec: BodySpec): { angle: number; kind: ArmKind }[] {
    const count = spec.arms || 8;
    const rows: { angle: number; kind: ArmKind }[] = [];
    for (let i = 0; i < count; i++) {
      const angle = (i / count) * Math.PI * 2;
      const sx = Math.sin(angle);
      const cz = Math.cos(angle);
      if (cz > (spec.skipFront ?? 0.78)) continue;
      let kind: ArmKind = 'back';
      // No BODIES entry sets flareAt or hangAt, so both comparisons were
      // `n > undefined`, i.e. always false, and every arm stays a 'back'. The
      // explicit undefined test is the same false, spelled out.
      if (spec.flareAt !== undefined && Math.abs(sx) > spec.flareAt) kind = 'flare';
      else if (spec.hangAt !== undefined && cz > spec.hangAt) kind = 'hang';
      rows.push({ angle, kind });
    }
    return rows;
  }

  setBody(name: string): string {
  ...
  _seatMouthMesh(mesh: THREE.Mesh, elevation: number, kind: string): void {
    const r = this.radius;
  ...
  placeEyes({ tilt = 0, spacing = 0.34, elevation = 0.1, lTilt = 0, rTilt = 0 }: PlaceEyesOptions = {}): void {
    const r = this.radius;
    const face = _face.set(0, elevation, 1).normalize();
  ...
  _seatEye(mesh: THREE.Object3D, face: THREE.Vector3, offset: number, radius: number, extraTilt = 0): void {
  ...
  setExpression(name: string): string {
  ...
  showMouth(hold: number = Infinity): void {
  ...
  setMouth(visible: boolean, hold?: number): void {
  ...
  update(delta: number, time?: number): void {
    ...
    // isOctopusBody takes nothing and always answers true; the argument it
    // was handed was never read. Same for _updateTentacles' second one.
    const lift = isOctopusBody() ? 0.1 : 0;
    if (lift) this.position.y += this.radius * lift;
    this._updateTentacles(t);
  }

  _updateTentacles(t: number): void {
    // The length test above means index 0 is there.
  ...
  _updateAnim(delta: number, t: number): void {
    const damp = (v: number, to: number) => THREE.MathUtils.damp(v, to, 6, delta);
    ...
    if (this.animation === 'excited') {
      ...
      if (phase < 0.14) {
        ...
        // hopI is Math.min(2, ...) of a non-negative floor, so 0, 1 or 2.
        yaw = sides[hopI]!;
    ...
    if (this.animation === 'dance') {
      ...
      let lean = 0;
      let yaw = 0;
      const step = (u: number) => Math.sin(Math.min(1, Math.max(0, u)) * Math.PI);
      if (phase < 0.12) {
        hop = step(phase / 0.12);
        lean = -0.14;
        yaw = -0.2;
...
export function createGrokBot(options?: GrokBotOptions): GrokBot {
