...
assert.match(desk, /hl=en/);
assert.match(desk, /--lang=en-US/);
assert.match(desk, /user-data-dir=/);
assert.match(desk, /kill_unprofiled_chrome/);
assert.match(desk, /ps -eo pid=/);
assert.match(desk, /gl=us/);
assert.match(desk, /accept_languages/);
assert.match(fs.readFileSync(path.join(root, "vm", "desk-init.sh"), "utf8"), /chrome-desk/);
assert.match(fs.readFileSync(path.join(root, "vm", "desk-init.sh"), "utf8"), /chown root:root \/tmp\/\.X11-unix/);
assert.match(fs.readFileSync(path.join(root, "vm", "desk-display.sh"), "utf8"), /chrome-desktop/);
assert.match(fs.readFileSync(path.join(root, "vm", "desk-display.sh"), "utf8"), /chown root:root \/tmp\/\.X11-unix/);
assert.match(fs.readFileSync(path.join(root, "vm", "Dockerfile"), "utf8"), /fonts-noto-core/);
assert.match(fs.readFileSync(path.join(root, "vm", "Dockerfile"), "utf8"), /chown root:root \/tmp\/\.X11-unix/);
