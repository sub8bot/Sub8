...
// Linux: the engine is a local system service, so a docker CLI whose daemon is
// merely stopped must RECOVER (`systemctl start docker`) — never a full
// get.docker.com reinstall over an install that is already there.
assert.equal(plan({ platform: "linux", cli: true, daemon: false }).engine, "moby");
assert.equal(plan({ platform: "linux", cli: true, daemon: false, colima: false, desktop: false }).action, "recover");
assert.equal(plan({ platform: "linux", cli: true, daemon: false, brew: true }).action, "recover");

// macOS/Windows: a BARE docker CLI with no engine behind it (`brew install
// docker` and nothing else) has nothing for recover to start, so it must fall
// through to installing a real engine rather than spinning on recover.
assert.equal(plan({ platform: "darwin", cli: true, daemon: false, colima: false, desktop: false, brew: true }).action, "brew-colima");
assert.equal(plan({ platform: "darwin", cli: true, daemon: false, colima: false, desktop: false, brew: false }).action, "static-colima");
assert.equal(plan({ platform: "win32", cli: true, daemon: false, desktop: false, winget: true }).action, "winget-desktop");
assert.equal(plan({ platform: "win32", cli: true, daemon: false, desktop: false, winget: false }).action, "download-desktop");
