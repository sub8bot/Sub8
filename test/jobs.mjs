...
import {
  applyStepUpdate,
  isChiefFollowUpReport,
  isMetaStepLabel,
  jobProgress,
  matchStepForAssignment,
  maybeFinalizeSummary,
  newJob,
  statusForExistingStep,
  taskTabName,
  uniqueMemberName,
  upsertJobStep,
...
assert.equal(statusForExistingStep("pending", "running"), "running");
assert.equal(statusForExistingStep("done", "running"), undefined);
assert.equal(statusForExistingStep("done", "pending"), undefined);
assert.equal(statusForExistingStep("blocked", "looping"), undefined);
assert.equal(statusForExistingStep("done", "done"), "done");
assert.equal(statusForExistingStep("blocked", "blocked"), "blocked");

const followJob = newJob({
  title: "triple",
  steps: [
    { label: "example.com h1", bot_id: "w1" },
    { label: "example.org h1", bot_id: "w3" },
    { label: "Summary", bot_id: "c" },
  ],
});
assert.equal(isChiefFollowUpReport(followJob), false);
applyStepUpdate(followJob, { botId: "w1", status: "done", detail: "Example Domain" });
assert.equal(isChiefFollowUpReport(followJob), false, "first worker done is not a follow-up until Summary is done");
applyStepUpdate(followJob, { botId: "w3", status: "done", detail: "Example Domain" });
applyStepUpdate(followJob, { label: "Summary", status: "done", detail: "compiled" });
assert.equal(isChiefFollowUpReport(followJob), true);
assert.equal(jobProgress(followJob).complete, true);
const reopened = statusForExistingStep(followJob.steps[0].status, "running");
const afterNote = upsertJobStep(followJob, {
  botId: "w1",
  content: "PONG-HARDER DISPLAY=:2",
  status: reopened,
});
assert.equal(afterNote.steps[0].status, "done", "follow-up note must not reopen a finished step");
assert.equal(afterNote.steps[0].detail, "Example Domain");
assert.equal(isChiefFollowUpReport(afterNote), true, "Summary still done → relay, do not recompile");
