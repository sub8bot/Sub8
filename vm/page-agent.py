...
def main(argv):
    ...
    try:
        ...
        if action == "select":
            if len(argv) < 4:
                print("usage: page-agent select REF OPTION", file=sys.stderr)
                return 1
            n, _ = node_at(ws, argv[2])
            want = " ".join(argv[3:])
            res = one.cdp_call(ws, "DOM.resolveNode", {"backendNodeId": int(n["backend"])})
            ...
            fn = (
                ...
                "const os=[...el.options];"
                "const opt=os.find(o=>o.value.toLowerCase()===w||o.label.trim().toLowerCase()===w||o.text.trim().toLowerCase()===w)"
                "||os.find(o=>o.text.trim().toLowerCase().includes(w));"
                "if(!opt)return 'no option matching '+want+' (have: '+os.map(o=>o.text.trim()).join(', ')+')';"
                "el.value=opt.value;"
            ...
            out = one.cdp_call(
                ...
                {"objectId": obj, "functionDeclaration": fn, "arguments": [{"value": want}], "returnByValue": True},
            )
            print((out.get("result") or {}).get("value", "select done"))
            return 0
        if action == "press":
            ...
            print(f"pressed {key}")
            return 0
        print("usage: page-agent snapshot|click REF|fill REF TEXT|select REF OPTION|navigate URL|press KEY", file=sys.stderr)
        return 1
    except Exception as err:
