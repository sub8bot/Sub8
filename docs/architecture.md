...
sequenceDiagram
  ...
  participant Store as @sub8/store
...
**Edit the `.mts` / `.ts`, never the `.mjs` / `.js` beside it.** `server/`, `web/`
and `electron/` are TypeScript. `tsc -b` emits the JavaScript *in place*, next to
each source, because Electron boots `electron/main.mjs`, tests import
`server/*.mjs`, and `index.html` loads `web/*.js` over plain URLs — so no import
specifier had to move. Those emitted files are gitignored and are overwritten on
every build: a change made to one is gone at the next `npm run build`.

That is why the paths below name sources. Run commands and the diagrams above
still name the emit (`node server/index.mjs`), which is correct — that is the
file Node actually executes.

| Path | Role |
|---|---|
| `electron/main.mts` | Desktop shell, spawn server, window, cache, data migration |
| `server/index.mts` | HTTP API, SSE, turn queue, static `web/` |
| `server/agent.mts` | Model client, system prompt, tool loop |
| `server/vm.mts` | Docker lifecycle, screenshot, input, stream proxy |
| `packages/desk-ports` | Display slots, the docker port map, harness stdio bridge (`@sub8/desk-ports`) |
| `packages/store` | `bots.json`, conversations, settings, channels, traces (`@sub8/store`) |
| `packages/automations` | Schedule parse, upsert, due routines (`@sub8/automations`) |
| `server/isolation.mts` | Host-path block, VM-only shell |
| `packages/control` | Human-is-driving flag, pauses agent input (`@sub8/control`) |
| `@sub8/store/trace` | JSONL traces under `data/traces/` |
| `server/paths.mts` | `appRoot`, unpacked files (re-exports `dataDir`) |
| `web/app.ts` | Main UI: rail, chat, settings, computer pane |
| `web/avatar.ts` | Shared Three.js renderer, mood, look sliders |
| `web/grok-bot.ts` | Procedural octopus: body, face, motion, color flash |
| `web/tool.html` | Avatar catalog |
| `web/palette.ts` | Shared color swatches |
...
flowchart TB
  ...
  agent[agent.mjs]
  vm[vm.mjs]
  routines["@sub8/automations"]
  iso[isolation.mjs]
  ctrl["@sub8/control"]
  trace["@sub8/store/trace"]
  paths[paths.mjs]
  ports["@sub8/desk-ports"]
...
- `data/channels.json` — rooms (`@sub8/store/channels`)
- `data/traces/<id>.jsonl` — tunnel traces (`@sub8/store/trace`)

Normalizes missing `avatar` to `{ expression, animation, body: "rounder" }`.
Owns `dataDir`; `server/paths.mjs` re-exports it and keeps `appRoot`/`fileRoot`.
...
- Starts noVNC; assigns a host port from 13100 up (the arithmetic — display
  slots, the `docker ps` port map, `:3011` — is `@sub8/desk-ports`)
...
Human control is in-memory only (`@sub8/control`). Restarting the server clears it.
